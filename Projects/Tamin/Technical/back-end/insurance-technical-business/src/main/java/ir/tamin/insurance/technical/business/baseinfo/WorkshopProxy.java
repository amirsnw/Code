...
//@Stateless
//@Named("WorkshopProxy")
//public class WorkshopProxy extends AbstractEntityProxy implements EntityProxy {
//
//    @Override
//    @ProxyMethod
...
//                value = f.getValue();
//            }
//            List<Workshop> w = null;
//            Long qcount = 0l;
//            Map<String, Object> dataPage = new HashMap<>();
//            String q = "select t from Workshop t ";
//            String qc = "select count(t) from Workshop t";
//            if (!"".equals(value)) {
//                value = value.replace("*", "");
//                q = q + " where   (t.id LIKE '%" + value + "%' or t.workshopName LIKE '%" + value + "%'" + ")";
//                qc = qc + " where (t.id LIKE '%" + value + "%' or t.workshopName LIKE '%" + value + "%'" + ")";
//            }
//            w = (List<Workshop>) entityManager.createQuery(q).setFirstResult(start).setMaxResults(limit).getResultList();
