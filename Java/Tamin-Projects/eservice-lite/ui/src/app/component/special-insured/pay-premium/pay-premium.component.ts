...
import { DisplayDetailPayPremiumComponent } from '../display-detail-pay-premium/display-detail-pay-premium.component';
...
export class PayPremiumComponent extends TaminPageBaseComponent {
  public overlay: any;
  ...
  @ViewChild('displayDetailPayPremiumComponent') displayDetailPayPremiumComponent: DisplayDetailPayPremiumComponent;
  ...
  initializePage() {
    ...
    setTimeout(() => {
      this.restService.getAll(Urls.getLastPayment)
        .then(data => {
          if (data.data !== undefined && data.data != null && data.data !== 0) {
            this.lastPaymentDate = data.data;
            this.hasPaymentHistory = true;
          } else if (data.data !== undefined && data.data != null && data.data === 0) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.restService.update(Urls.checkContractStatus, null, null)
        .then(data => {
  ...
  onShowPaymentHistory() {
    this.redirectTo('/optional-insurance/display-detail-pay-premiume');
  }
  ...
  onCalcPremium() {
    ...
    this.restService.getAll(`${Urls.calcDebit}/${month}`)
      .then(data => {
  ...
  onDisplayCalcDetail() {
    ...
    this.displayDetailPayPremiumComponent.show(this.startDate, this.endDate);
  }

  onPayPremium() {
    this.overlay = this.showOverlay();
    this.restService.getAll(`${Urls.onlinePayment}?start-date=${this.startDate}&end-date=${this.endDate}&amount=${this.total}&systemType=01`)
      .then(data => {
        this.hideOverlay(this.overlay);
        if (data !== undefined && data != null) {
