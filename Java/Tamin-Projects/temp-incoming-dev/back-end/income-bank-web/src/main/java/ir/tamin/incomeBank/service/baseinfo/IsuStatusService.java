...
public class IsuStatusService {
    ...
    private List<IsuStatus> getList(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) {

        TypedQuery query = entityManager.createQuery(getQuery(filter, sort));

        List<IsuStatus> isuStatusList = new ArrayList<>();

        if (start != null && limit != null) {
            isuStatusList = query.setFirstResult(start).setMaxResults(limit).getResultList();
        } else if (start == null && limit != null) {
            isuStatusList = query.setMaxResults(limit).getResultList();
        } else if (start != null && limit == null) {
            isuStatusList = query.setFirstResult(start).getResultList();
        } else if (start == null && limit == null) {
            isuStatusList = query.getResultList();
        }
        return isuStatusList;
    ...
    private CriteriaQuery getQuery(FilterWrapper filterWrapper, SortWrapper sort) {
        try {
            ...
            Root<IsuStatus> isuStatusFrom = query.from(IsuStatus.class);
            Metamodel metamodel = entityManager.getMetamodel();
            EntityType<IsuStatus> isuStatusEntityType = metamodel.entity(IsuStatus.class);
            ...
            if (filterWrapper != null && filterWrapper.getFilters() != null) {
                for (Filter filter : filterWrapper.getFilters()) {
                    ...
                    switch (operator) {
                        case LIKE:
                            predicates.add(criteriaBuilder.like(isuStatusFrom.get(isuStatusEntityType.getDeclaredSingularAttribute(field, String.class)), "%" + value + "%"));
                            break;
                        case EQUAL:
                            javax.persistence.criteria.Path path = isuStatusFrom.get(f[0]);
            ...
            if (sort != null) {
                ...
                for (Sort sortSet : sort.getSortSet()) {
                    ...
                    javax.persistence.criteria.Path path = isuStatusFrom.get(sortProperties[0]);
                    ...
                    if (sortSet.getDirection().getName().equals(Sort.Direction.DESC.getName())) {
                        order = criteriaBuilder.desc(isuStatusFrom.get(sortProperties[0]));
                    } else {
                        order = criteriaBuilder.asc(isuStatusFrom.get(sortProperties[0]));
                    }
                    orders.add(order);
                }
            ...
            query.select(isuStatusFrom);
            return query;
        } catch (Exception ex) {
            Logger.getLogger(IsuStatus.class.getName()).log(Level.SEVERE, ex.getMessage(), ex);
            return null;
        }
    }
