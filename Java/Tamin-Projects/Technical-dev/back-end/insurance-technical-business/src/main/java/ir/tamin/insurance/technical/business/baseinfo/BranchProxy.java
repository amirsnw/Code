...
@Stateless
public class BranchProxy extends AbstractEntityProxy implements EntityProxy {
    ...
    public CollectionData search(Class clazz, FilterWrapper fw, SortWrapper sw, Integer start, Integer limit, boolean includeCount) throws ProxyProcessingException {
        if (fw != null) {
            ...
            if (!"".equals(value)) {
                value = value.replace("*", "");
                q = q + " where   (t.brhCode LIKE '%" + value + "%' or t.brhName LIKE '%" + value + "%'" + ")";
                qc = qc + " where (t.brhCode LIKE '%" + value + "%' or t.brhName LIKE '%" + value + "%'" + ")";
            }
