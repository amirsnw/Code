...
import {TaminDataGridConfigurationFactory, TaminFieldAutoCompleteDataGridComponent, TaminLazyLoadService, TaminPageBaseComponent} from 'tamin-framework';
...
@Component({
  selector: 'app-all-sso-health-center-locations',
  templateUrl: './all-sso-health-center-locations.component.html',
  styleUrls: ['./all-sso-health-center-locations.component.css']
})
export class AllSsoHealthCenterLocationsComponent extends TaminPageBaseComponent {
  private mapRef: any;
  id: any;
  map: any;
  _overlay: any;
  ...
  protected loadPageData(): void {
    ...
    this.restService.getAll(Urls.MAP_ALL_GEO_MEDICAL)
      .then(value => {
  ...
  private setPosition(location: any) {
    ...
    this.restService.getAll(`${Urls.MAP_ALL_GEO_MEDICAL_VEHICLE}?lat1=${location.latlng.lat}&lon1=${location.latlng.lng}&flag=1`)
      .then(value => {
        this.directionRefVehicle = L.polyline(this.decodePath(value.data.points, false), {color: 'red', weight: 6}).addTo(me.map);
        this.restService.getAll(`${Urls.MAP_ALL_GEO_MEDICAL_FOOT}?lat1=${location.latlng.lat}&lon1=${location.latlng.lng}&flag=1`)
          .then(value1 => {
