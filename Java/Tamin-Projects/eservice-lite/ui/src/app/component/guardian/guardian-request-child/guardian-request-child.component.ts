...
@Component({
  selector: 'app-guardian-request-child',
  templateUrl: './guardian-request-child.component.html',
  styleUrls: ['../guardian.css']
})
export class GuardianRequestChildComponent extends TaminPageBaseComponent {
  theForm: FormGroup;
  ...
  protected initializePage(): void {
    ...
    this.theForm = this.formBuilder.group({
      requestDate: [''],
      nationalIdChild: ['', [Validators.required, Validators.maxLength(10), Validators.minLength(10), TaminValidators.nationalId]],
      fullNameChild: [''],
      birthDateChild: ['', Validators.required],
      ageChild: [''],
      requestStatus: [''],
      insuranceStatusChild: [''],
      type: ['', ],
  ...
  loadPageData() {
    ...
    this.theForm.get('requestType').setValue(4);
    ...
    this.theForm.get('nationalIdChild').valueChanges.subscribe(value => {
      if (this.theForm.get('nationalIdChild').valid && this.theForm.get('birthDateChild').valid) {
    ...
    this.theForm.get('birthDateChild').valueChanges.subscribe(value => {
      if (this.theForm.get('birthDateChild').valid && this.theForm.get('nationalIdChild').valid) {
        this.isDisabledOffice = false;
      } else {
        this.isDisabledOffice = true;
        this.isDisabledConfirm = true;
  ...
  getOfficeData(): Promise<any> {
    ...
    let url;
    if (values.nationalIdChild !== '') {
      url = values.nationalIdChild + '/' + momentNs.from(new Date(values.birthDateChild).toString(), 'en').locale('fa').format('YYYYMMDD') + '/' + '03' + '/' + '0' + '/' + 0 + '/' + '06';
    } else {
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      this.restService.getAll(theUrl, searchParams)
        .then(value => {
          ...
          if (value.data.registrationOfficeSon !== undefined) {
            this.firstNameChild = value.data.registrationOfficeSon.firstName;
            this.lastNameChild = value.data.registrationOfficeSon.lastName;
            this.theForm.get('fullNameChild').setValue(value.data.registrationOfficeSon.firstName + ' ' + value.data.registrationOfficeSon.lastName);
            this.theForm.get('ageChild').setValue(value.data.registrationOfficeSon.age);
            this.theForm.get('insuranceStatusChild').setValue(value.data.insuredStatusSon === '1' ? 'دارای ارتباط بیمه ای' : 'فاقد ارتباط بیمه ای');
            this.manageForm(value.data.registrationOfficeSon.age, value.data.insuredStatusSon);
          }
          resolve();
        }).catch(reason => {
        this.hideOverlay(this._overlay);
  ...
  saveData() {
    ...
    this._overlay = this.showOverlay();
    if (new Date(values.birthDateChild).getTime() > new Date().getTime()) {
      this.showErrorMessageBox('خطا', 'تاریخ تولد از تاریخ روز نمی تواند بزرگتر باشد.');
      return;
    }
    const data = {
    ...
    if (this.theForm.get('nationalIdChild').value !== '') {
      data.guardianRelationlistList.push(
        {
          ...
          firstName: this.firstNameChild,
          lastName: this.lastNameChild,
          birthDate: momentNs.from(new Date(this.theForm.get('birthDateChild').value).toString(), 'en').locale('fa').format('YYYYMMDD'),
          age: this.theForm.get('ageChild').value,
          natinoalCode: this.theForm.get('nationalIdChild').value,
          isinsured: this.theForm.get('insuranceStatusChild').value === 'بیمه پرداز می باشد' ? '1' : '0',
          depperrelTypeCode: '03',
        }
      );
    }
    ...
    this.restService.create(this.isSSO ? `${Urls.GUARDIAN_REQUEST_SSO}/${this.ssoNationalCode}/${this.ssoTicket}`
      : Urls.GUARDIAN_REQUEST, data).then(value => {
        this.hideOverlay(this._overlay);
        this.showInfoMessageBox('پیام سیستم', 'اطلاعات فرزند تحت کفالت با موفقیت ثبت و در نوبت رسیدگی شعبه قرار گرفت و نتیجه توسط پیامک به شما اطلاع رسانی می گردد و همچنین جهت مشاهده وضعیت درخواست می توان به تامین من مراجعه فرمایید.', () => {
          this.redirectTo('/');
          setTimeout(function () {
            location.reload();
          }, 500);
