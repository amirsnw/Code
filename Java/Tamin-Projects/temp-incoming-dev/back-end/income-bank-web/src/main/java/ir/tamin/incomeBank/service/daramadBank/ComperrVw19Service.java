...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw19;
 ...
 * @author h_riazat
 */
public class ComperrVw19Service {
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR19(?,?,?,?)}");
        ...
        List<ComperrVw19> comperrList = new ArrayList<>();
        while (rs.next()) {
            ComperrVw19 comperrModel = new ComperrVw19();
            comperrModel.setId(rs.getString("row_id"));
            comperrModel.setRcsChqNo(rs.getString("RCS_CHQNO"));
            comperrModel.setRcsChqDat(rs.getString("RCS_CHQDAT"));
            comperrModel.setRcsChqAmt(rs.getLong("RCS_CHQAMT"));
            comperrModel.setCardPrice(rs.getLong("CARD_PRICE"));
            comperrModel.setCheckAmel(rs.getString("CHECKAMEL"));
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        cell4.setCellValue("مبلغ در کارت بانک");
        cell4.setCellStyle(headerStyle);
        Cell cell5 = row.createCell(5);
        cell5.setCellValue("ردیف بانک در بردرور");
        cell5.setCellStyle(headerStyle);
        Cell cell6 = row.createCell(6);
        cell6.setCellValue("ردیف بانک در کارت بانک");
        cell6.setCellStyle(headerStyle);
        try {
            Map<String, Object> map = getAll(filterWrapper, null, null, null);
            List<ComperrVw19> comperrList = (List<ComperrVw19>) map.get("list");
            int rownum = 1;
            for (ComperrVw19 item : comperrList) {
                String rcsChqNo = item.getRcsChqNo();
                String rcsChqDat = (item.getRcsChqDat() != null ? item.getRcsChqDat().substring(0, 4) + "/" + item.getRcsChqDat().substring(4, 6) + "/" + item.getRcsChqDat().substring(6, 8) : " ");
                Long rcsChqAmt = item.getRcsChqAmt();
                Long cardPrice = item.getCardPrice();
                String checkAmel = item.getCheckAmel();
                ...
                cell = row.createCell(cellnum++);
                cell.setCellStyle(detailStyle);
                cell.setCellValue(checkAmel);
                cell = row.createCell(cellnum++);
                cell.setCellStyle(detailStyle);
                cell.setCellValue(cardRadif);
            }
        } catch (SQLException ex) {
            Logger.getLogger(ComperrVw19Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
    ...
    public Database writeToAccess(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) throws ClassNotFoundException, SQLException, IOException, NumberFormatException {
        String file = "comperrReport19.accdb";
        Map<String, Object> map = getAll(filter, null, null, null);
        List<ComperrVw19> comperrList = (List<ComperrVw19>) map.get("list");
        try {
            Database db = (DatabaseBuilder.create(Database.FileFormat.V2010, new File(file)));
            String comperrReport19 = "comperrReport19";
            Table table = new TableBuilder(comperrReport19)
                    .addColumn(new ColumnBuilder("RCS_CHQNO").setSQLType(Types.VARCHAR).toColumn())
                    .addColumn(new ColumnBuilder("RCS_CHQDAT").setSQLType(Types.VARCHAR).toColumn())
                    .addColumn(new ColumnBuilder("RCS_CHQAMT").setSQLType(Types.NUMERIC).toColumn())
                    .addColumn(new ColumnBuilder("CARD_PRICE").setSQLType(Types.NUMERIC).toColumn())
                    .addColumn(new ColumnBuilder("CHECKAMEL").setSQLType(Types.VARCHAR).toColumn())
                    .addColumn(new ColumnBuilder("CARD_RADIF").setSQLType(Types.VARCHAR).toColumn())
                    .toTable(db);
            for (ComperrVw19 item : comperrList) {
                String rcsChqNo = item.getRcsChqNo();
                String rcsChqDat = item.getRcsChqDat();
                Long rcsChqAmt = item.getRcsChqAmt();
                Long cardPrice = item.getCardPrice();
                String checkAmel = item.getCheckAmel();
                String cardRadif = item.getCardRadif();
                table.addRow(rcsChqNo, rcsChqDat, rcsChqAmt, cardPrice, checkAmel, cardRadif);
            }
            db.close();
            return db;
        } catch (Exception ex) {
