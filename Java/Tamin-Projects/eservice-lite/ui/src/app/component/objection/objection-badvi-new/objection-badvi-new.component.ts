...
import {ObjectionDebitNewComponent} from '../objection-debit-new/objection-debit-new.component';
...
@Component({
  selector: 'app-objection-badvi-new',
  templateUrl: './objection-badvi-new.component.html',
  styleUrls: ['./objection-badvi-new.component.css']
})
export class ObjectionBadviNewComponent extends TaminPageBaseComponent {

  theForm: FormGroup;
  @ViewChild('imageGallery') imageGallery: TaminImageGalleryManagedComponent;
  // @ViewChild('imageGallery2') imageGallery2: TaminImageGalleryManagedComponent;
  ...
  protected initializePage(): void {
    ...
    // this.imageGallery2.saveUrl = Urls.UploadImage;
    // this.imageGallery2.getUrl = Urls.UploadImage;
  ...
  loadPageData() {
    ...
    this.restService.getById(Urls.WorkshopsInfo, this.workshopId + '/' + this.brchCode)
      .then(value => {
        ...
        this.restService.getAll(Urls.ObjectionWorkshopsDebit + '/' + this.workshopId + '/' + this.brchCode, this.searchParams)
          .then(result => {
          ...
          .catch(reason => {
            ...
            if (reason.error && reason.error.data) {
              this.showErrorMessageBox('پیام سیستم', reason.error.data.message, () => {
                this.redirectTo('/objection-debit');
              });
            } else {
  ...
  // addImage2() {
  //   this.imageGallery2.selectImage('بارگزاری لایحه اعتراض سازمان به رای بدوی', '19');
  // }
  ...
  onSendRequest() {
    ...
    // this.imageGallery2.images.forEach(value => {
    //   switch (value.tag) {
    //     case '19':
    //       data.type19 = '1';
    //       break;
    ...
    // });
    this.overlay = this.showOverlay();
    this.restService.create(Urls.ObjectionSave, data)
      .then(value => {
        this.hideOverlay(this.overlay);
