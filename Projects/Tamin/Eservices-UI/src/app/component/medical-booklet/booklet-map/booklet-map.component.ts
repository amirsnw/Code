...
import {TaminLazyLoadService, TaminModalComponent, TaminPageBaseComponent, SearchParam, SearchOperator,OverlayService} from 'tamin-framework';
...
@Component({
  selector: 'app-booklet-map',
  templateUrl: './booklet-map.component.html',
  styleUrls: ['./booklet-map.component.css']
})
export class BookletMapComponent extends TaminPageBaseComponent {
  ...
  getCenter(cityCode: string): Promise<Array<number>> {
    return new Promise<Array<number>>((resolve, reject) => {
      if (cityCode === null || cityCode === undefined || cityCode === '') {
       ...
       //  Urls.BRANCH_GEO,
       // '?',
       // `filter=[{property=cityCode,operator=EQUAL, value=${cityCode}}]`
         //'lat1=',
         //lat.toString(),
         //'&',
         //'lon1=',
