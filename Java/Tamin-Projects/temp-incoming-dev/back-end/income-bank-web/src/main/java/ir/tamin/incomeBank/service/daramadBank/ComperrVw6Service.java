...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw6;
 ...
 * @author f_fotuhi
 */
public class ComperrVw6Service {
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR6(?,?,?,?)}");
        ...
        List<ComperrVw6> comperrList = new ArrayList<>();
        while (rs.next()) {
            ComperrVw6 comperrModel = new ComperrVw6();

            comperrModel.setId(rs.getString("row_id"));
            comperrModel.setPayId(rs.getString("pay_id"));
            comperrModel.setErrDesc(rs.getString("err_desc"));
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        try {
            ...
            List<ComperrVw6> comperrList = (List<ComperrVw6>) map.get("list");
            int rownum = 1;
            for (ComperrVw6 item : comperrList) {
                String payId = item.getPayId();
                String errDesc = item.getErrDesc();
                ...
                cell = row.createCell(cellnum++);
                cell.setCellStyle(detailStyle);
                cell.setCellValue(errDesc);

            }

        } catch (SQLException ex) {
            Logger.getLogger(ComperrVw6Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
    ...
    public Database writeToAccess(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) throws ClassNotFoundException, SQLException, IOException, NumberFormatException {
        ...
        String file = "comperrReport6.accdb";
        Map<String, Object> map = getAll(filter, null, null, null);
        List<ComperrVw6> comperrList = (List<ComperrVw6>) map.get("list");

        try {
            Database db = (DatabaseBuilder.create(Database.FileFormat.V2010, new File(file)));
            String comperrReport6 = "comperrReport6";
            Table table = new TableBuilder(comperrReport6) // Creating table
                    .addColumn(new ColumnBuilder("PAY_ID").setSQLType(Types.VARCHAR).toColumn())
                    .addColumn(new ColumnBuilder("ERR_DESC").setSQLType(Types.VARCHAR).toColumn())
                    .toTable(db);
            for (ComperrVw6 item : comperrList) {

                String payId = item.getPayId();
                String errDesc = item.getErrDesc();
                table.addRow(payId, errDesc);//Inserting values into the table
            }
            db.close();
            return db;
        } catch (Exception ex) {
