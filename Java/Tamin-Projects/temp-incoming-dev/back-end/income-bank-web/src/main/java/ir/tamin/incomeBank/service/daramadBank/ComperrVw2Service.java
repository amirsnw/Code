...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw2;
 ...
 * @author f_fotuhi
 */
public class ComperrVw2Service {
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR2(?,?,?,?)}");
        ...
        List<ComperrVw2> comperrList = new ArrayList<>();
        while (rs.next()) {
            ComperrVw2 comperrModel = new ComperrVw2();
            ...
            comperrModel.setSum(rs.getString("sum"));
            comperrModel.setDif(rs.getString("tafavot"));
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        cell1.setCellValue("شماره پرداخت");
        ...
        cell7.setCellValue("مجموع مبلغ");
        cell7.setCellStyle(headerStyle);

        Cell cell8 = row.createCell(8);
        cell8.setCellValue("مابه التفاوت");
        cell8.setCellStyle(headerStyle);

        try {
            Map<String, Object> map = getAll(filterWrapper, null, null, null);
            List<ComperrVw2> comperrList = (List<ComperrVw2>) map.get("list");
            int rownum = 1;
            for (ComperrVw2 item : comperrList) {
                ...
                Long sum = (item.getSum() != null ? Long.valueOf(item.getSum()) : null);
            ...
            Logger.getLogger(ComperrVw2Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
    ...
    public Database writeToAccess(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) throws ClassNotFoundException, SQLException, IOException, NumberFormatException {
        ...
        String file = "comperrReport2.accdb";
        Map<String, Object> map = getAll(filter, null, null, null);
        List<ComperrVw2> comperrList = (List<ComperrVw2>) map.get("list");

        try {
            Database db = (DatabaseBuilder.create(Database.FileFormat.V2010, new File(file)));
            String comperrReport2 = "comperrReport2";
            Table table = new TableBuilder(comperrReport2) // Creating table
                    .addColumn(new ColumnBuilder("PAY_ID").setSQLType(Types.VARCHAR).toColumn())
                    .addColumn(new ColumnBuilder("ORD_DOC_DAT").setSQLType(Types.VARCHAR).toColumn())
                    ...
                    .addColumn(new ColumnBuilder("SUM").setSQLType(Types.NUMERIC).toColumn())
                    .addColumn(new ColumnBuilder("DIF").setSQLType(Types.NUMERIC).toColumn())
                    .toTable(db);
            for (ComperrVw2 item : comperrList) {
                ...
                Long sum = (item.getSum() != null ? Long.valueOf(item.getSum()) : null);
                Long dif = (item.getDif() != null ? Long.valueOf(item.getDif()) : null);
                table.addRow(payId, ordDocDat, rcvDate, rcvDesc, rcvNo, orpPaySeqAmt, sum, dif);//Inserting values into the table
            }
            db.close();
            return db;
        } catch (Exception ex) {
