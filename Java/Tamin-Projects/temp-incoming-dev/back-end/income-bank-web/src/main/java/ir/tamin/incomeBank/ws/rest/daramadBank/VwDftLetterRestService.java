...
import ir.tamin.incomeBank.service.daramadBank.VwDftLetterService;
...
@RequestScoped
public class VwDftLetterRestService implements ResourceRESTService<VwDftLetter> {

    @Inject
    VwDftLetterService vwDftLetterService;
    ...
    public Response getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sortWrapper, UriInfo uriInfo, SecurityContext securityContext, HttpServletRequest request) throws WebApplicationException {
        return ResponseHelper.ok(vwDftLetterService.getAll(filterWrapper, start, limit, sortWrapper));
    }

    @Override
    public Response save(VwDftLetter t, UriInfo uriInfo, SecurityContext securityContext, HttpServletRequest request) throws WebApplicationException {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public Response edit(String id, VwDftLetter t, UriInfo uriInfo, SecurityContext securityContext, HttpServletRequest request) throws WebApplicationException {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
    ...
    public Response exportToPdf(@Context SecurityContext context, @Context HttpServletRequest request) throws IOException {
        User user = userBean.getUserByUserName(context.getUserPrincipal()
                .getName());
        byte[] byteStream = vwDftLetterService.exportToPdf(request, user);
        return Response
                .ok(byteStream)
