Ext.define('InsuranceTechnical.view.insAgreement.InsuranceAgreementSpecController', {
    ...
    init: function () {
        ...
        var searchParams = InsuranceTechnical.getApplication().getCache('searchParams');
        me.getViewModel().set('brchCode', orgCode);
        if (orgCode !== undefined && orgCode !== '0000') {
            Ext.getCmp('branchCode').disable();
        }
        if (searchParams) {
            var form = Ext.getCmp('insAgreement-spec-search-form').getForm();
            Ext.Object.each(searchParams, function (property, value) {
                if (/^[1-4]\d{3}\/((0[1-6]\/((3[0-1])|([1-2][0-9])|(0[1-9])))|((1[0-2]|(0[7-9]))\/(30|([1-2][0-9])|(0[1-9]))))$/.test(value)) {
                    value = InsuranceTechnical.tamin.helpers.Persian.gregorianToPersian(new Date(value))
                }
                form.findField(property).setValue(value);
            });
            me.onSearchButton();
        }
    ...
    onShowAllButton: function (btn) {
        ...
        InsuranceTechnical.getApplication().removeCache('searchParams');
        var store = me.getViewModel().getStore('insuranceAgreementSpecStore');
        store.clearFilter(true);
        store.load();
    },
    ...
    onEditButton: function (view, rowIndex, colIndex, item, e, rec, row, action) {
        ...
        var form = Ext.getCmp('insAgreement-spec-search-form');
        var formValues = form.getValues();
        InsuranceTechnical.getApplication().addCache('searchParams', formValues);
    ...
    onMedicalResultButton: function (btn) {
        ...
        Ext.getCmp('medicalLetterDate').setMaxValue(new Date());
        win.show();
        win.center();
    },
