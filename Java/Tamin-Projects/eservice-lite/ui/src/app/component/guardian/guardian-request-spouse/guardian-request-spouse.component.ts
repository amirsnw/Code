...
@Component({
  selector: 'app-guardian-request-spouse',
  templateUrl: './guardian-request-spouse.component.html',
  styleUrls: ['../guardian.css']
})
export class GuardianRequestSpouseComponent extends TaminPageBaseComponent {
  theForm: FormGroup;
  ...
  protected initializePage(): void {
    ...
    this.theForm = this.formBuilder.group({
      requestDate: [''],
      nationalIdSpouse: ['', [Validators.required, Validators.maxLength(10), Validators.minLength(10), TaminValidators.nationalId]],
      fullNameSpouse: [''],
      birthDateSpouse: ['', Validators.required],
      ageSpouse: [''],
      requestStatus: [''],
      insuranceStatusSpouse: [''],
      type: ['', ],
  ...
  loadPageData() {
    ...
    this.theForm.get('requestType').setValue(5);
    ...
    this.theForm.get('nationalIdSpouse').valueChanges.subscribe(value => {
      if (this.theForm.get('nationalIdSpouse').valid && this.theForm.get('birthDateSpouse').valid) {
    ...
    this.theForm.get('birthDateSpouse').valueChanges.subscribe(value => {
      if (this.theForm.get('birthDateSpouse').valid && this.theForm.get('nationalIdSpouse').valid) {
        this.isDisabledOffice = false;
      } else {
        this.isDisabledOffice = true;
        this.isDisabledConfirm = true;
  ...
  getOfficeData(): Promise<any> {
    ...
    let url;
    if (values.nationalIdSpouse !== '') {
      url = values.nationalIdSpouse + '/' + momentNs.from(new Date(values.birthDateSpouse).toString(), 'en').locale('fa').format('YYYYMMDD') + '/' + '02' + '/' + '0' + '/' + 0 + '/' + '06';
    } else {
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      this.restService.getAll(theUrl, searchParams)
        .then(value => {
          ...
          if (value.data.registrationOfficeSpouse !== undefined) {
            this.firstNameSpouse = value.data.registrationOfficeSpouse.firstName;
            this.lastNameSpouse = value.data.registrationOfficeSpouse.lastName;
            this.theForm.get('fullNameSpouse').setValue(value.data.registrationOfficeSpouse.firstName + ' ' + value.data.registrationOfficeSpouse.lastName);
            this.theForm.get('ageSpouse').setValue(value.data.registrationOfficeSpouse.age);
            this.theForm.get('insuranceStatusSpouse').setValue(value.data.insuredStatusSpouse === '1' ? 'دارای ارتباط بیمه ای' : 'فاقد ارتباط بیمه ای');
            this.manageForm(value.data.registrationOfficeSpouse.age, value.data.insuredStatusSpouse);
          }
          resolve();
        }).catch(reason => {
        this.hideOverlay(this._overlay);
  ...
  saveData() {
    ...
    this._overlay = this.showOverlay();
    if (new Date(values.birthDateSpouse).getTime() > new Date().getTime()) {
      this.showErrorMessageBox('خطا', 'تاریخ تولد از تاریخ روز نمی تواند بزرگتر باشد.');
      return;
    }
    const data = {
    ...
    if (this.theForm.get('nationalIdSpouse').value !== '') {
      data.guardianRelationlistList.push(
        {
          ...
          firstName: this.firstNameSpouse,
          lastName: this.lastNameSpouse,
          birthDate: momentNs.from(new Date(this.theForm.get('birthDateSpouse').value).toString(), 'en').locale('fa').format('YYYYMMDD'),
          age: this.theForm.get('ageSpouse').value,
          natinoalCode: this.theForm.get('nationalIdSpouse').value,
          isinsured: this.theForm.get('insuranceStatusSpouse').value === 'بیمه پرداز می باشد' ? '1' : '0',
    ...
    this.restService.create(this.isSSO ? `${Urls.GUARDIAN_REQUEST_SSO}/${this.ssoNationalCode}/${this.ssoTicket}`
      : Urls.GUARDIAN_REQUEST, data).then(value => {
        this.hideOverlay(this._overlay);
        this.showInfoMessageBox('پیام سیستم', 'اطلاعات همسر تحت کفالت با موفقیت ثبت و در نوبت رسیدگی شعبه قرار گرفت و نتیجه توسط پیامک به شما اطلاع رسانی می گردد و همچنین جهت مشاهده وضعیت درخواست می توان به تامین من مراجعه فرمایید.', () => {
          this.redirectTo('/');
          setTimeout(function () {
            location.reload();
          }, 500);
