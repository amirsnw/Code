...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw5;
 ...
 * @author f_fotuhi
 */
public class ComperrVw5Service {
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR5(?,?,?,?)}");
        ...
        List<ComperrVw5> comperrList = new ArrayList<>();
        while (rs.next()) {
            ComperrVw5 comperrModel = new ComperrVw5();
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        cell6.setCellValue("تاریخ واریزی");
        ...
        try {
            ...
            List<ComperrVw5> comperrList = (List<ComperrVw5>) map.get("list");
            int rownum = 1;
            for (ComperrVw5 item : comperrList) {
            ...
            Logger.getLogger(ComperrVw5Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
    ...
    public Database writeToAccess(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) throws ClassNotFoundException, SQLException, IOException, NumberFormatException {
        ...
        String file = "comperrReport5.accdb";
        Map<String, Object> map = getAll(filter, null, null, null);
        List<ComperrVw5> comperrList = (List<ComperrVw5>) map.get("list");

        try {
            Database db = (DatabaseBuilder.create(Database.FileFormat.V2010, new File(file)));
            String comperrReport5 = "comperrReport5";
            Table table = new TableBuilder(comperrReport5) // Creating table
            ...
            for (ComperrVw5 item : comperrList) {
                ...
                String rcvNo = item.getRcvNo();
                Long cardPrice = (item.getCardPrice() != null ? Long.valueOf(item.getCardPrice()) : null);
                table.addRow(cardOrdPay, ordDocDat, cardDate, rcvDesc, rcvNo, rcvDate, cardPrice);//Inserting values into the table
            }
            db.close();
            return db;
        } catch (Exception ex) {
