...
export class CommitteeStageThreeHistoryComponent extends TaminPageBaseComponent {
  ...
  protected initializePage(): void {
    this.hideDownloadForm4 = this.mainModel.ninteenOneChecked || this.mainModel.lightWork;

    if (this.mainModel.formCase === 1 && !this.mainModel.isFamily) {
      this.form4Desc = 'دانلود فرم استعلام از پزشک';
      this.form4Type = 'A';
    } else if (this.mainModel.isFamily) {
      this.form4Desc = 'دانلود فرم استعلام از پزشک افراد تبعی بیمه شدگان / مستمری بگیران اصلی';
      this.form4Type = 'B';
    } else if (this.mainModel.formCase === 3) {
      this.form4Desc = 'دانلود فرم استعلام از پزشک بازماندگان بیمه شدگان اصلی متوفی';
      this.form4Type = 'C';
    }
    ...
    if (!this.mainModel.ninteenOneChecked) {
  ...
  addDoctorHistory() {
    this.doctorFormValidationError = false;
    const missingStatus = !this.doctorHistoryGallery.areAllImagesUploaded();
    if (!this.doctorHistoryForm.touched || !this.doctorHistoryForm.valid || missingStatus) {
    ...
    switch (actionName) {
      ...
      case 'upload':
        ...
        this.hosDocTypeCombo.dataItems = [
          ...
          {'title': `نامه رسمی بیمارستان به استعلام فرم شماره 5 (حداکثر یک برگ) (بیمارستان ${param.item.hospitalName})`, 'tag': '168'},
        ];
      ...
      if (response.data == null) {
        ...
        if (AppHelper.isWeb()) {
          this.pdfFormGenerator.downloadForm4PdfDesktop(this.diseaseForm.value.requestNumber, this.mainModel.stageOneFormOne.value.commNationalCode,
            ...
            this.mainModel.demandInfoId, this.form4Type, this);
        } else {
