...
@Mapping(type = MappingType.COMPLEX, input = OccurCauseReportInput.class, output = OccurCauseReportValue.class)
public class OccurCauseReport  extends AbstractDBFunction {
    @Inject
    private Logger logger;
    private static final String QUERY = "{? = call pckssupviews.get_vwrepssup_occur_cause (?, ?, ?)}";


    @Override
    public DBFunctionCall createCall() {
        try {
            OccurCauseReportInput input = (OccurCauseReportInput) inputResource;
