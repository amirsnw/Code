...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw8;
 ...
 * @author m_salami
 */
public class ComperrVw8Service {
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR8(?,?,?,?)}");
        ...
        List<ComperrVw8> vw8List = new ArrayList<>();
        while (rs.next()) {
            ComperrVw8 ComperrVw8Model = new ComperrVw8();

            ComperrVw8Model.setPayId(rs.getString("pay_id"));
            ComperrVw8Model.setOrdDocdat(rs.getString("ord_docdat"));
            ComperrVw8Model.setRcvDesc(rs.getString("rcv_desc"));
            ComperrVw8Model.setRcvNo(rs.getString("rcv_no"));
            ComperrVw8Model.setRcvDate(rs.getString("rcv_date"));
            ComperrVw8Model.setOrpPayseqamt(rs.getLong("orp_payseqamt"));

            vw8List.add(ComperrVw8Model);
        }

        rs.close();
        map.put("list", vw8List);
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        try {
            ...
            List<ComperrVw8> comperrList = (List<ComperrVw8>) map.get("list");
            int rownum = 1;
            for (ComperrVw8 item : comperrList) {
            ...
            Logger.getLogger(ComperrVw8Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
