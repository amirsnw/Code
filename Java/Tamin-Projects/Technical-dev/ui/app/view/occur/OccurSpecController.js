...
Ext.define('InsuranceTechnical.view.occur.OccurSpecLController', {
    ...
    onAddButton: function (button) {
        ...
        var insStore = this.getStore('insuranceRegisterations');
        if (!win) {
        ...
        insStore.loadData([], false);
        insStore.clearData(true);
        win.show();
        win.center();
    },
    ...
    saveOccurNewButton: function (btn) {
        ...
        var data = {
            ...
            pnatcode: personalData.pnatcode,
            ...
            insuranceSpec: {
                id: personalData.id,
                brchCode: personalData.insBrchCode
            },
            brchCode: {
                branchCode: personalData.insBrchCode
            },
            rwshBranch: {
                branchCode: personalData.insBrchCode
            }
    ...
    findID: function (field, newValue) {
        ...
        if (!(insBrchCode = me.getViewModel().get('personalData.insBrchCode'))) {
            insBrchCode = orgCode;
        }
        me.resetIdentityForm();
        if (newValue === null) {
                ...
                callback: function (record, operation, success) {
                    ...
                    if (success) {
                        if (record.length !== 0) {
                            ...
                            if (!record) {
                                me.insRegPopUpHandler(field);
                                Ext.Msg.alert('پیام سیستم', 'نامنویسی بیمه شده در شعبه کاربر جاری یافت نشد، لازم است از نامنویسی شعب دیگر اطلاعات بیمه شده را انتخاب نمایید.');
                                return;
                            }

                            me.getViewModel().set('personalData.brchCode', record.getData().brchCode);
                            me.getViewModel().set('personalData.firstName', record.getData().firstName);
                            me.getViewModel().set('personalData.lastName', record.getData().lastName);
                            me.getViewModel().set('personalData.pnatcode', record.getData().nationalId);
                            me.getViewModel().set('personalData.pnationdesc', record.getData().nation === '01'
                                ? 'ایرانی' : record.getData().nation);
                            me.getViewModel().set('personalData.pfathername', record.getData().fatherName);
                            me.getViewModel().set('personalData.pidno', record.getData().idCardNumber);
                            var birthDate = record.getData().dateOfBirth;
                            me.getViewModel().set('personalData.mainPbirthdate', birthDate);
                            me.getViewModel().set('personalData.pbirthdate',
                                birthDate.substring(0, 4) + '/' + birthDate.substring(4, 6) + '/' + birthDate.substring(6, 8));
                            me.getViewModel().set('personalData.pexpcityname', record.getData().cityOfBirthDesc);
                            me.getViewModel().set('personalData.sexcode', record.getData().gender);
                            me.getViewModel().set('personalData.insBrchCode', record.getData().brchCode);
    ...
    insRegPopUpHandler: function (buttom) {
        var me = this;
        var win = me.lookupReference('InsuredPersonPopup');
        if (!win) {
            win = Ext.create('InsuranceTechnical.view.main.InsuredPersonPopup');
            me.resetIdentityForm(true);
            me.getView().add(win);
        }
        win.setCallback(function () {
            if (win.selectedItem !== null) {
                me.getViewModel().set('personalData.insBrchCode',
                    win.selectedItem.data.brchCode);
                buttom.setValue(win.selectedItem.data.id);
            }
        });
        win.show();
    },
    ...
    onSelectRecordWorkshop: function (btn) {
        ...
        if (data.length !== null || data.length !== 0 || data.length !== 'undefind') {
            ...
            btn.up('window').selectedItem = selection;
            btn.up('window').close();
        }
    },
    ...
    workshopPopUpHandler: function (buttom) {
        var me = this;
        var insuredBrchCode = me.getViewModel().get('personalData.insBrchCode');
        if (!insuredBrchCode || insuredBrchCode === '') {
            Ext.Msg.alert('پیام سیستم', 'ابتدا اطلاعات بیمه شده را وارد نمایید.');
            return;
        }
        var win = me.lookupReference('WorkshopOccurPopup');
        if (!win) {
        ...
        win.setCallback(function () {
            if (win.selectedItem !== null) {
                buttom.setValue(win.selectedItem.data.workshopId);
            }
        });
        win.show();
    },
    ...
    editOccurButton: function (view, rowIndex, colIndex, item, e, rec, row, action) {
        ...
        me.getViewModel().set('personalData.insBrchCode', recordData.brchCode.branchCode);
    ...
    onEditOccurButton: function (btn) {
        ...
        var personalData = me.getViewModel().get('personalData');
        me.getViewModel().set('user', user);
        me.getViewModel().set('branchCode', user.organization.code);
        ...
        win.mask('در حال ویرایش اطلاعات ...');
        Ext.Ajax.request({
            url: url,
            method: 'PUT',
            jsonData: data,
    ...
    OnOccurOtherBrhLetterButton: function (btn) {
        ...
        var win = me.lookupReference('occur-other-brh-letter-ref');
        var user = InsuranceTechnical.getApplication().getCache('user');
    ...
    saveNewOccurIdea: function (btn) {
        ...
        var win = this.lookupReference('new-occur-idea-ref');
    ...
    saveOccurInspConfButton: function (btn) {
        ...
        var win = this.lookupReference('occur-insp-conf-ref');
    ...
    printOccurReviewReports: function (button) {
        var me = this;
        var user = InsuranceTechnical.getApplication().getCache('user');
    ...
    saveOccurWorkInspButton: function (btn) {
        ...
        if (activeRecord.inspworksended === '1') {
            data.workinspletdate = activeRecord.workinspletdate instanceof Date
                ? activeRecord.workinspletdate.getTime() : new Date(activeRecord.workinspletdate).getTime();
        } else {
            data.workinspletdate = null;
        }
        win.mask('در حال ذخيره اطلاعات ...');
    ...
    resetOccurForm: function (forAll) {
        ...
        this.getViewModel().set('workshopData.workshopName', '');
        this.getViewModel().set('workshopData.isuJobLocation', '');
        this.getViewModel().set('workshopData.branchCode', '');
