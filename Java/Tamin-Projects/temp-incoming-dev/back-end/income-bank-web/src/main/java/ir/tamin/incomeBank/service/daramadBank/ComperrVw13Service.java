...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw13;
 ...
 * @author m_salami
 */
public class ComperrVw13Service {
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR13(?,?,?,?)}");
        ...
        List<ComperrVw13> vw13List = new ArrayList<>();
        while (rs.next()) {
            ComperrVw13 ComperrVw13Model = new ComperrVw13();

            ComperrVw13Model.setCardOrdpay(rs.getString("card_ordpay"));
            ComperrVw13Model.setCardDate(rs.getString("card_date"));
            ComperrVw13Model.setCardPrice(rs.getLong("card_price"));

            vw13List.add(ComperrVw13Model);
        }

        rs.close();
        map.put("list", vw13List);
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        try {
            ...
            List<ComperrVw13> comperrList = (List<ComperrVw13>) map.get("list");
            int rownum = 1;
            for (ComperrVw13 item : comperrList) {
            ...
            Logger.getLogger(ComperrVw13Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
    ...
    public Database writeToAccess(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) throws ClassNotFoundException, SQLException, IOException, NumberFormatException {
        ...
        String file = "comperrReport13.accdb";
        Map<String, Object> map = getAll(filter, null, null, null);
        List<ComperrVw13> comperrList = (List<ComperrVw13>) map.get("list");

        try {
            Database db = (DatabaseBuilder.create(Database.FileFormat.V2010, new File(file)));
            String comperrReport13 = "comperrReport13";
            Table table = new TableBuilder(comperrReport13) // Creating table
