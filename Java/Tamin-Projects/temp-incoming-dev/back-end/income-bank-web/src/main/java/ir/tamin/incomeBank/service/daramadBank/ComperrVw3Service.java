...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw3;
 ...
 * @author f_fotuhi
 */
public class ComperrVw3Service {
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR3(?,?,?,?)}");
        ...
        List<ComperrVw3> comperrList = new ArrayList<>();
        while (rs.next()) {
            ComperrVw3 comperrModel = new ComperrVw3();
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        try {
            ...
            List<ComperrVw3> comperrList = (List<ComperrVw3>) map.get("list");
            int rownum = 1;
            for (ComperrVw3 item : comperrList) {
            ...
            Logger.getLogger(ComperrVw3Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
    ...
    public Database writeToAccess(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) throws ClassNotFoundException, SQLException, IOException, NumberFormatException {
        ...
        String file = "comperrReport3.accdb";
        Map<String, Object> map = getAll(filter, null, null, null);
        List<ComperrVw3> comperrList = (List<ComperrVw3>) map.get("list");

        try {
            Database db = (DatabaseBuilder.create(Database.FileFormat.V2010, new File(file)));
            String comperrReport3 = "comperrReport3";
            Table table = new TableBuilder(comperrReport3) // Creating table
            ...
            for (ComperrVw3 item : comperrList) {
                ...
                String rcvNo = item.getRcvNo();
                Long orpPaySeqAmt = (item.getOrpPaySeqAmt() != null ? Long.valueOf(item.getOrpPaySeqAmt()) : null);
                table.addRow(payId, ordDocDat, rcvDate, rcvDesc, rcvNo, orpPaySeqAmt);//Inserting values into the table
            }
            db.close();
            return db;
        } catch (Exception ex) {
