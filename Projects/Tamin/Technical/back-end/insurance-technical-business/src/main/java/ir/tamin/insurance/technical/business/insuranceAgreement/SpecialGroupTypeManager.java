...
@Stateless
@Named("SpecialGroupTypeManager")
public class SpecialGroupTypeManager extends AbstractEntityProxy implements EntityProxy {
    ...
    public CollectionData search(Class clazz, FilterWrapper fw, SortWrapper sw, Integer start, Integer limit, boolean includeCount) throws ProxyProcessingException {
        ...
        collectionData.getList().forEach(item -> {
            SpecialGroupType identity = (SpecialGroupType) item;
            if (identity.getSpecialGroupCode() != null) {
                String activityCode = (String) entityManager.createNamedQuery("SpecialActivity.getCatBySpecialId")
                        .setParameter("specialCategoryId", identity.getSpecialGroupCode())
                        .getSingleResult();
                if (activityCode != null) {
                    identity.setActivityCode(activityCode);
                }else {
                    String subCode = (String) entityManager.createNamedQuery("SpecialActivity.getSubBySpecialId")
                            .setParameter("specialCategoryId", identity.getSpecialGroupCode())
                            .getSingleResult();
                    identity.setSubCode(subCode);
                }
            }
        });
        //}
