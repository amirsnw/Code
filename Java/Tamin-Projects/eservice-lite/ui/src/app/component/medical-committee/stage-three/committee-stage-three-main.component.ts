  ...
  onRequestAction(param: any) {
    ...
    switch (actionName) {
      ...
      case 'edit':
        ...
        const doctorRecordsItems = param.item.committeeRequestInfoDocumentList.filter(value => {
          return value.documentTypeId === '171';
        });
        ...
        this.documentComponent.documentForm.patchValue({
          ...
          doctorRecords: doctorRecordsItems.length > 0,
        ...
        for (const item of pathologyReportItems) {
          this.documentComponent.documentGallery.downloadImage(item.documentFileId, 'گواهی پزشک معالج', item.documentTypeId, '0', true, item.documentTypeId);
        }
