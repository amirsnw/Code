...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw14;
 ...
 * @author m_salami
 */
public class ComperrVw14Service {
    @Inject
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR14(?,?,?,?)}");
        ...
        List<ComperrVw14> vw14List = new ArrayList<>();
        while (rs.next()) {
            ComperrVw14 ComperrVw14Model = new ComperrVw14();

            ComperrVw14Model.setPayId(rs.getString("pay_id"));
            ComperrVw14Model.setOrdDocdat(rs.getString("ord_docdat"));
            ComperrVw14Model.setOrpPayseqamt(rs.getLong("orp_payseqamt"));

            vw14List.add(ComperrVw14Model);
        }

        rs.close();
        map.put("list", vw14List);
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        try {
            ...
            List<ComperrVw14> comperrList = (List<ComperrVw14>) map.get("list");
            int rownum = 1;
            for (ComperrVw14 item : comperrList) {
            ...
            Logger.getLogger(ComperrVw14Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
    ...
    public Database writeToAccess(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) throws ClassNotFoundException, SQLException, IOException, NumberFormatException {
        ...
        String file = "comperrReport14.accdb";
        Map<String, Object> map = getAll(filter, null, null, null);
        List<ComperrVw14> comperrList = (List<ComperrVw14>) map.get("list");

        try {
            Database db = (DatabaseBuilder.create(Database.FileFormat.V2010, new File(file)));
            String comperrReport14 = "comperrReport14";
            Table table = new TableBuilder(comperrReport14) // Creating table
                    ...
                    .addColumn(new ColumnBuilder("ORPPAYSEQAMT").setSQLType(Types.NUMERIC).toColumn())
                    .toTable(db);
            for (ComperrVw14 item : comperrList) {
                ...
                table.addRow(payId, ordDocdat, orpPayseqamt);//Inserting values into the table
            }
            db.close();
            return db;
        } catch (Exception ex) {
