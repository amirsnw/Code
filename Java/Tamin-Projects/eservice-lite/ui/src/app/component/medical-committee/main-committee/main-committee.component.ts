...
export class MainCommitteeComponent extends TaminPageBaseComponent {
  ...
  changeStage() {
    switch (this.stage) {
      case 1:
        this.componentRef.instance.stageValidateAndCache.then(valid => {
  ...
  onStageOneSubmit() {
    ...
    this.model.ninteenOneChecked = jsondata.demandTypeCode === '18';
    this.model.lightWork = jsondata.demandTypeCode === '04';
...
حداکثر تا 10 روز، نتیجه بررسی به شما اعلام خواهد شد.`, () => {
  ...
  private checkFinalImagesMissingStatus() {
    ...
    for (const documentItem of this.uploadTypeCombo.dataItems) {
      ...
      if (!(this.model.ninteenOneChecked && documentItem.type === '25')
