...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw17;
 ...
 * @author h_riazat
 */
public class ComperrVw17Service {
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR17(?,?,?,?)}");
        ...
        List<ComperrVw17> comperrList = new ArrayList<>();
        while (rs.next()) {
            ComperrVw17 comperrModel = new ComperrVw17();
            ...
            comperrModel.setRcsRcvSubAmt(rs.getLong("rcs_rcvsubamt"));
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        cell2.setCellValue("شماره چک");
        cell2.setCellStyle(headerStyle);
        Cell cell3 = row.createCell(3);
        cell3.setCellValue("مبلغ ریز چک");
        cell3.setCellStyle(headerStyle);
        try {
            Map<String, Object> map = getAll(filterWrapper, null, null, null);
            List<ComperrVw17> comperrList = (List<ComperrVw17>) map.get("list");
            int rownum = 1;
            for (ComperrVw17 item : comperrList) {
            ...
            Logger.getLogger(ComperrVw17Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
    ...
    public Database writeToAccess(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) throws ClassNotFoundException, SQLException, IOException, NumberFormatException {
        String file = "comperrReport17.accdb";
        Map<String, Object> map = getAll(filter, null, null, null);
        List<ComperrVw17> comperrList = (List<ComperrVw17>) map.get("list");
        try {
            Database db = (DatabaseBuilder.create(Database.FileFormat.V2010, new File(file)));
            String comperrReport17 = "comperrReport17";
            Table table = new TableBuilder(comperrReport17)
                    ...
                    .addColumn(new ColumnBuilder("RCS_RCVSUBAMT").setSQLType(Types.NUMERIC).toColumn())
                    .toTable(db);
            for (ComperrVw17 item : comperrList) {
                String payId = item.getPayId();
                String rcsChqNo = item.getRcsChqNo();
                Long rcsRcvSubAmt = item.getRcsRcvSubAmt();
                table.addRow(payId, rcsChqNo, rcsRcvSubAmt);
            }
            db.close();
            return db;
        } catch (Exception ex) {
