...
export class CommitteeCaseOneComponent extends TaminPageBaseComponent implements OnDestroy {
  ...
  initSubscribes() {
    ...
    this.formFour.get('demandTypeCode').valueChanges.subscribe(value => {
      if (value === '18') {
        ...
        const birthDateParam = new SearchParam();
        birthDateParam.value = new Date(this.dataInput.birthDate).getTime().toString();
        birthDateParam.operator = SearchOperator.EQ;
        birthDateParam.property = 'birthDate';
        searchParams.push(birthDateParam);
  ...
  get stageValidateAndCache() {
    ...
    if (!this.formOne.valid || !this.formTwo.valid || !this.formThree.valid
      ...
      this.validateHistoryRate().then(done => {
