...
@Component({
  selector: 'app-debit-investigation',
  templateUrl: './debit-investigation.component.html',
  styleUrls: ['./debit-investigation.component.css']
})
export class DebitInvestigationComponent extends TaminPageBaseComponent {
  theForm: FormGroup;
  ...
  onSendRequest() {
    ...
    const data = {
      ...
      objectionType: '3',
      objectionDate: '',
      mastCustomerTypeCode: this.data.mastCustomerTypeCode,
      peymanSequence: this.data.peymanSequence,
      debitStartDate: this.data.debitStartDate,
      debitEndDate: this.data.debitEndDate,
      debitStepCode: this.data.debitStepCode,
      bimehAmount: this.data.bimehAmount,
      bikariAmount: this.data.bikariAmount,
      jarimehAmount: this.data.jarimehAmount,
      sayerAmount: this.data.sayerAmount,
      orderNumber: this.data.docNoEjra,
      orderDate: this.data.docDateEjra,
      eblaghDate: this.data.docDateEblaghEjra,
      kindDoc: this.data.kindDoc
    };
    ...
    this.showQuestionBox('پیام سیستم', 'آیا برای ثبت درخواست اطمینان دارید؟', () => {
      this.overlay = this.showOverlay();
      this.restService.create(Urls.DebitComitteSave, data)
        .then(value => {
          this.hideOverlay(this.overlay);
          this.showInfoMessageBox('پیام سیستم', 'درخواست شما با شماره پیگیری ' + '<b>' + value.data.refId + '</b>' + ' با موفقیت ذخیره گردید و بعد از رسیدگی، نتیجه از طریق پیامک یا منو درخواست های من اعلام می شود.', () => {
            this.redirectTo('/debit-management');
          });
        })
        .catch(reason => {
