Ext.define('IncomeBank.view.common.TcrOrganizationSelectionController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.tcrOrganizationSelection',
    ...
    loadTreeData: function () {
        ...
        me.getViewModel().getStore('SelectedOrganization').on('remove', function (store, records, index, isMove, eOpts) {
            ...
            var root = Ext.getCmp('tcr-organization-selection-tree').getRootNode();
    ...
    buildOrganizationTreeData: function () {
        var me = this;
        Ext.Ajax.request({
            url: IncomeBank.helper.Urls.getUrl('getAllTcrOrganizations'),
            method: 'GET',
            success: function (response1, opts1) {
                var TcrOrganizations = JSON.parse(response1.responseText).data.list;
                var TcrOrganizationsArray = [];
                for (var i = 0; i < TcrOrganizations.length; i++) {
                    TcrOrganizationsArray.push(TcrOrganizations[i].organizationCode);
                }
                me.getViewModel().set("TcrOrganizations", TcrOrganizationsArray);
                /*-----------------------------------------------------------------------------------*/
                Ext.Ajax.request({
                    url: IncomeBank.helper.Urls.getUrl('getAllTcrProvinces'),
                    method: 'GET',
        ...
        var selectAll = Ext.Array.map(root, function (item0, index, array) {
            var obj = Ext.Array.map(provinces, function (item, index, array) {
                ...
                var tmp1 = Ext.Array.map(tmp, function (item, index, array) {
                    ...
                    result.leaf = true;
                    result.id = item.organizationCode;
