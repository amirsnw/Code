...
@Component({
  ...
  templateUrl: './conscription.component.html',
  styleUrls: ['./conscription.component.css']
})
export class ConscriptionComponent extends TaminPageBaseComponent {
  ...
  protected loadPageData(): void {
    ...
    this.checkEligibility().then(done => {
      if (this.firstState) {
        this.restService.getAll(Urls.Soldier_Wage)
          .then(wageValue => {
            if (wageValue.data.list.length <= 1) {
              ...
              return this.restService.getAll(Urls.Soldier_Duration)
                .then(value => {
                  if (value.data.list < 1) {
                    ...
                    const amountTemp = Math.round(parseFloat(((wageAverageTemp * 7 / 100) * monthDurationTemp).toFixed(2)));
  ...
  private checkEligibility() {
    ...
    return new Promise<any>((resolve, reject) => {
      this.restService
        .getAll(Urls.Soldier_Validation, searchParams)
        .then(result => {
          ...
          if (this.middleState) {
            ...
            return this.restService
              .getAll(Urls.Soldier_Installment)
              .then(item => {
                ...
                return this.restService.getAll(Urls.SoldierFinalDetails).then(value => {
  ...
  private onSend() {
    ...
    this._overlay = this.showOverlay();
    this.restService.getAll(`${Urls.onlinePayment}?start-date=${''}&end-date=${''}&amount=${totalTemp}&installmentCount=${installmentCountTemp}&systemType=${'soldier'}`)
      .then(data => {
  ...
  private _initializeDataGrid() {
    this.dataGrid.configuration = (new TaminDataGridConfigurationFactory())
      ...
      .clearSortParams()
      .clearVisibleColumns()
      // .addUrl(Urls.Soldier_Installment)
      .setShowPager(true)
      .setFirstLoad(true)
