...
@Component({
  selector: 'app-marriage-no-presence_menu',
  templateUrl: './marriage-no-presence_menu.component.html',
  styleUrls: ['./marriage-no-presence_menu.component.css']
})
export class MarriageNoPresenceMenuComponent extends TaminPageBaseComponent {
  ...
  onSendRequest() {
    ...
    const data = {
      shorttermRequest:
      {
        ...
        nationalCode:null,// tmp.nationalCode,
        requestHelpType: '05',
  ...
  onValidateRequest() {
    ...
    this._overlay = this.showOverlay();
    const theUrl = StpUrls.STP_VERIFY_MARRIAGE_Date + '/' + marDate + '/' + spouseNationalId;
