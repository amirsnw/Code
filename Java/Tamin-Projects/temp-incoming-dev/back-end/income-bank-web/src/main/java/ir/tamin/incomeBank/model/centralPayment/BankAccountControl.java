 ...
 */
@Entity
@Table(name = "CP_BANK_ACCOUNT_CONTROL")
@NamedQueries({
    @NamedQuery(name = "BankAccountControl.getByNatCode", query = "select d from BankAccountControl d where d.natcode=:natcode "),
    @NamedQuery(name = "BankAccountControl.getByNatcodeAcc", query = "select d from BankAccountControl d where d.natcode =:natcode AND d.accountNo = :accountNo AND d.bankCode = :bankCode ")
})
