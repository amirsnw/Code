...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw18;
 ...
 * @author h_riazat
 */
public class ComperrVw18Service {
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR18(?,?,?,?)}");
        ...
        List<ComperrVw18> comperrList = new ArrayList<>();
        while (rs.next()) {
            ComperrVw18 comperrModel = new ComperrVw18();
            comperrModel.setId(rs.getString("row_id"));
            comperrModel.setCardOrdPay(rs.getString("CARD_ORDPAY"));
            comperrModel.setCardRcvNo(rs.getString("CARD_RCVNO"));
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        try {
            ...
            List<ComperrVw18> comperrList = (List<ComperrVw18>) map.get("list");
            int rownum = 1;
            for (ComperrVw18 item : comperrList) {
            ...
            Logger.getLogger(ComperrVw18Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
    ...
    public Database writeToAccess(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) throws ClassNotFoundException, SQLException, IOException, NumberFormatException {
        String file = "comperrReport18.accdb";
        Map<String, Object> map = getAll(filter, null, null, null);
        List<ComperrVw18> comperrList = (List<ComperrVw18>) map.get("list");
        try {
            Database db = (DatabaseBuilder.create(Database.FileFormat.V2010, new File(file)));
            String comperrReport18 = "comperrReport18";
            Table table = new TableBuilder(comperrReport18)
            ...
            for (ComperrVw18 item : comperrList) {
                ...
                String cardRcvNo = item.getCardRcvNo();
                Long cardPrice = item.getCardPrice();
                table.addRow(cardOrdPay, cardRcvNo, cardPrice);
            }
            db.close();
            return db;
        } catch (Exception ex) {
