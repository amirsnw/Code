...
import ir.tamin.incomeBank.model.daramadBank.ComperrVw22;
 ...
 * @author h_riazat
 */
public class ComperrVw22Service {
    ...
    public Map<String, Object> getAll(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException, SQLException {
        ...
        procedure.query("{?=call pckdrmdview.getVWDRMDPAYERR22(?,?,?,?)}");
        ...
        List<ComperrVw22> comperrList = new ArrayList<>();
        while (rs.next()) {
            ComperrVw22 comperrModel = new ComperrVw22();
            comperrModel.setId(rs.getString("row_id"));
            comperrModel.setLetterSerial(rs.getString("LETTER_SERIAL"));
            comperrModel.setLetterPrice1(rs.getLong("LETTER_PRICE1"));
            comperrModel.setLetterPrice2(rs.getLong("LETTER_PRICE2"));
            comperrModel.setLetterDate(rs.getString("LETTER_DATE"));
    ...
    public Workbook writeToExcel(FilterWrapper filterWrapper, Integer start, Integer limit, SortWrapper sort) throws IOException {
        ...
        cell1.setCellValue("شماره مسلسل اعلامیه");
        cell1.setCellStyle(headerStyle);
        Cell cell2 = row.createCell(2);
        cell2.setCellValue("مبلغ حساب 1");
        cell2.setCellStyle(headerStyle);
        Cell cell3 = row.createCell(3);
        cell3.setCellValue("مبلغ حساب 2");
        cell3.setCellStyle(headerStyle);
        Cell cell4 = row.createCell(4);
        cell4.setCellValue("تاریخ اعلامیه");
        cell4.setCellStyle(headerStyle);
        try {
            Map<String, Object> map = getAll(filterWrapper, null, null, null);
            List<ComperrVw22> comperrList = (List<ComperrVw22>) map.get("list");
            int rownum = 1;
            for (ComperrVw22 item : comperrList) {
                String letterSerial = item.getLetterSerial();
                Long letterPrice1 = item.getLetterPrice1();
                Long letterPrice2 = item.getLetterPrice2();
                String letterDate = (item.getLetterDate() != null ? item.getLetterDate().substring(0, 4) + "/" + item.getLetterDate().substring(4, 6) + "/" + item.getLetterDate().substring(6, 8) : " ");
                ...
                cell.setCellStyle(detailStyle);
                cell.setCellValue(letterSerial);
                cell = row.createCell(cellnum++);
                cell.setCellStyle(detailStyle);
                cell.setCellValue(letterPrice1);
                cell = row.createCell(cellnum++);
                cell.setCellStyle(detailStyle);
                cell.setCellValue(letterPrice2);
                cell = row.createCell(cellnum++);
                cell.setCellStyle(detailStyle);
                cell.setCellValue(letterDate);
            }
        } catch (SQLException ex) {
            Logger.getLogger(ComperrVw22Service.class.getName()).log(Level.SEVERE, null, ex);
        }
        return wb;
    }
    ...
    public Database writeToAccess(FilterWrapper filter, Integer start, Integer limit, SortWrapper sort) throws ClassNotFoundException, SQLException, IOException, NumberFormatException {
        String file = "comperrReport22.accdb";
        Map<String, Object> map = getAll(filter, null, null, null);
        List<ComperrVw22> comperrList = (List<ComperrVw22>) map.get("list");
        try {
            Database db = (DatabaseBuilder.create(Database.FileFormat.V2010, new File(file)));
            String comperrReport22 = "comperrReport22";
            Table table = new TableBuilder(comperrReport22)
                    .addColumn(new ColumnBuilder("LETTER_SERIAL").setSQLType(Types.VARCHAR).toColumn())
                    .addColumn(new ColumnBuilder("LETTER_PRICE1").setSQLType(Types.NUMERIC).toColumn())
                    .addColumn(new ColumnBuilder("LETTER_PRICE2").setSQLType(Types.NUMERIC).toColumn())
                    .addColumn(new ColumnBuilder("LETTER_DATE").setSQLType(Types.VARCHAR).toColumn())
                    .toTable(db);
            for (ComperrVw22 item : comperrList) {
                String letterSerial = item.getLetterSerial();
                Long letterPrice1 = item.getLetterPrice1();
                Long letterPrice2 = item.getLetterPrice2();
                String letterDate = item.getLetterDate();
                table.addRow(letterSerial, letterPrice1, letterPrice2, letterDate);
            }
            db.close();
            return db;
        } catch (Exception ex) {
